resource "aws_lb" "web_alb" { #This weblication load balancer with http only, not https protocol, is only accessible if VPN is on, not accessible if VPN off.
  name ="${var.project_name}-${var.environment}-web-alb" #weblication load balancer
  internal           = false

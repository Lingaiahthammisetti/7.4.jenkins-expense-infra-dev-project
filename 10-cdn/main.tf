resource "aws_cloudfront_distribution" "web_cdn" {
  origin {
    domain_name              = "web-${var.environment}.${var.zone_name}" #web-dev.lingaiah.online
  ...
  enabled             = true
  aliases             = [ "web-${var.common_tags.Component}.${var.zone_name}" ]#web-cdn.lingaiah.online
...
module "records" {
  ...
  records = [
    {
      name    = "web-cdn" #web-cdn.lingaiah.online
      type    = "A"
